...
The chapter source files are stored in the `chapters/` folder. This keeps the root directory clean while allowing the Quarto book navigation to list the chapters in their teaching order.
...
The book source is organized around four teaching asset folders:

```text
chapters/   lecture-note chapter pages
htmls/      interactive standalone HTML modules
labs/       Jupyter notebook labs for Python and simulation
assets/     CSS and other supporting files
```

Because chapter files are inside `chapters/`, links from chapters to labs and HTML modules use relative paths such as `../labs/...` and `../htmls/...`. The top-level pages, including this introduction and the labs summary page, use `labs/...` and `htmls/...`.
